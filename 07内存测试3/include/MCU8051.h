...
{
    uint8_t *code_64KB;                   // 代码区 (用指针指向)
    uint8_t iram_low_128B[MEM_IRAM_SIZE]; // 内部数据存储区区 低128B
    uint8_t sfr_high_128B[MEM_SFR_SIZE];  // 特殊功能寄存器区 高128B
    uint8_t xram_64KB[MEM_XRAM_SIZE];     // 外部数据存储区区 64KB==65536B
} memory_t;
